...
| Index.ets 多组件拆分 | 第二轮已完成：日历、历史、操作菜单、欢迎、加载、关于、导入和确认窗口已拆出；响应式页面壳待推进 |
...
当前 `Index.ets` 负责页面状态、Preferences 和导入流程编排；基础展示与浮动窗口组件已通过带默认值的 `@Prop` 与回调接入，不直接读写本地数据。下一步由响应式页面壳决定单列、紧凑和 Pura X 展开横向双排表现。
