...
| Index.ets 多组件拆分 | 第一轮已完成：日历、历史统计、操作菜单和欢迎窗口已拆出；响应式页面壳待推进 |
...
├── components/       # 日历、历史统计、操作菜单和欢迎窗口
└── parser/           # JSON 校验；文本/OCR 暂缓
```

当前 `Index.ets` 负责页面状态、Preferences 和导入确认流程；基础展示组件已通过 `@Prop` 与回调接入，不直接读写本地数据。下一步由响应式页面壳决定单列、紧凑和 Pura X 展开横向双排表现。
