...
状态：当前实现快照（2026-08-17）。

本目录记录当前工程真实代码的组件、数据模型、通信方式和运行流程。它是重构后的当前边界基线，不把理想中的分层描述成已经存在的代码。组件解析、阶段 0-6 的代码整理和阶段 13-17 的运行边界复核均已完成；当前只保留导出写入失败这一受模拟器权限与选择器路径限制的 E2E 边界。
...
- [重构边界与观察项](./refactor-boundaries.md)：列出已执行的代码清理、当前保留的高耦合点和必须保持的 UI/功能约束。
...
- 阶段 0-6 的纯逻辑基线、数据协议收束、Index 编排拆分、展示层边界核对和无用代码清理已完成。
- 阶段 13-17 的运行边界复核已完成：Pura X 覆盖 compact/single/dual，Pura 90 覆盖 scroll；窗口解绑已做逐项异常隔离；导出写入失败的 URI 注入阻断已记录。

待分析：无。

全部组件、数据、入口和全局引用分析以及计划中的代码整理已完成；测试分层、运行证据和剩余边界见[初步重构计划](./10-refactor-plan.md)。

每一项均记录代码目的、输入输出、调用关系和明确的整理边界；后续变更必须先更新对应边界并保持现有 UI/功能基线。
