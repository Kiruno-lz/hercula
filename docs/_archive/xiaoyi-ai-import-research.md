...
月迹曾规划三阶段历史导入：JSON 文件、文本语义转严格 JSON、拍照识别文字后再转严格 JSON。当前只实现并维护 JSON schema 匹配；后两阶段暂时搁置，不进入当前功能开发或应用依赖。
...
| 2. 文本语义 | 规则解析或 Natural Language Kit | 暂时搁置 | 重新立项后验证 API 24 可用性、离线性、返回结构和隐私边界 |
| 3. 图片识别 | OCR 提取文本，再复用阶段 2 | 暂时搁置 | 重新立项后验证图片入口、Core Vision Kit 版本、端侧执行和性能 |
...
3. 断开网络，先在 Pura X 模拟器运行；真机仅在模拟器无法覆盖时补充。
...
- 任何未来模型输出都必须先转成 `json_SPEC.md` 定义的严格候选结构，经过用户确认后才能写入。
